...
| Timed effects | No buffs/debuffs system | Temporary effects with auto-expiration (potions, poisons, spells) |
...
2. Add `rate_modifier` from character stat (default 0) to `per_hour`
3. If `blocked_by` conditions are met and effective rate > 0, zero the rate
4. Apply effective rate to the stat
5. Clamp to `[min, max]`

**Formula:** `effective_rate = base_per_hour + rate_modifier + sum(effect_rate_bonus)`
If any `blocked_by` condition is true and rate > 0 → `effective_rate = 0`
      ...
      "rules": [
        {
          ...
          "blocked_by": [
            {"stat": "<stat_name>", "operator": "<=", "value": <number>}
          ]
        }
      ],
...
- `blocked_by` is a list of `{stat, operator, value}` objects. If ANY condition is true and the effective rate is positive, the rate is zeroed. Negative rates (drains) are never blocked.
...
```json
{
  "custom_stats": {
    "<stat_name>": {
      ...
      "min": 0,
      "max": 100,
      "rate_modifier": <number, optional, default 0>
    }
  }
}
```
...
```bash
# Add a timed buff (anti-radiation for 4 hours)
dm-survival.sh effect add "Антирадин" --stat radiation --rate-bonus -10 --duration 4

# Add healing over time (stackable)
dm-survival.sh effect add "Хилка" --stat hp --per-hour 5 --duration 3 --stackable

# Poison with instant damage + ongoing effect
dm-survival.sh effect add "Яд" --stat hp --instant -10 --stat radiation --rate-bonus 3 --duration 6

# Remove effect
dm-survival.sh effect remove "Яд"

# List active effects
dm-survival.sh effects
```

Three effect types: `--rate-bonus` (temporary rate modifier), `--per-hour` (direct change each hour), `--instant` (one-time on application). Effects auto-expire during tick. `--stackable` allows multiple instances of the same named effect; without it, a new application replaces the existing one.

### Rate modifiers

```bash
# Add modifier (clothing, potions, shelter)
dm-survival.sh rate warmth +5

# Set absolute modifier
dm-survival.sh rate warmth set +8

# Reset to base rate
dm-survival.sh rate warmth reset

# Show all effective rates
dm-survival.sh rates
```
...
```
.claude/additional/modules/custom-stats/
