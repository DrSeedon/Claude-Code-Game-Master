...
class MassCombatEngine:
    ...
    def _runtime(self) -> ModuleRuntime:
        """Create the neutral module runtime lazily for lightweight tests."""
        if not hasattr(self, "_module_runtime"):
            self._module_runtime = ModuleRuntime(
                project_root=PROJECT_ROOT,
                campaign_dir=self.campaign_dir,
            )
        return self._module_runtime

    def _describe_profile(self, entity: Dict) -> str:
        description = self._runtime().call_provider(
            "combat.profile.describe",
            entity,
        )
    ...
    def _resolve_damage(
        ...
        """Delegate optional damage semantics through the neutral provider API."""
        result = self._runtime().call_provider(
            "combat.damage",
            raw_damage,
            attacker,
            defender,
        ...
        return int(result["damage"]), str(result.get("details", ""))
    ...
    def add_units(
        ...
        combat_profile: Optional[Dict] = None,
    ) -> str:
        self._load()
        ...
        for i in range(count):
            ...
            if combat_profile:
                unit_data["combat_profile"] = dict(combat_profile)
            if unit_range:
    ...
    def add_from_template(
        ...
        return self.add_units(
            ...
            combat_profile=tmpl.get("combat_profile"),
        )
    ...
    def list_templates(self) -> str:
        ...
        for tid, t in self.templates.items():
            ...
            profile = self._describe_profile(t)
            profile_str = f" {C.c(profile, C.BCYAN)}" if profile else ""
            lines.append(
                ...
                f"[{C.c(tgt, tgt_c)}]{rng_tag}{profile_str} {C.c(notes, C.DIM)}"
            )
        return "\n".join(lines)
    ...
    def turret_fire(
        ...
        return self.aoe_damage(
            ...
            source_profile=turret.get("combat_profile"),
        )
    ...
    def _spray_fire(
        ...
        for tgt_uid in targets:
            ...
            dmg_after_armor, scaling = self._resolve_damage(
                dmg,
                source_unit,
                target,
            )
            ...
            profile_str = f" {C.c(scaling, C.BCYAN)}" if scaling else ""
            ...
            lines.append(
                f"  🔫 → {C.c(tgt_uid, C.BOLD)}: [{rolls_str}]{mod_str}={dmg}{profile_str}{save_str}"
    ...
    def aoe_damage(
        ...
        for tname in target_names:
            ...
            dmg_after_armor, scaling = self._resolve_damage(
                total,
                {"combat_profile": source_profile or {}},
                target,
            )
            ...
            hp_c = C.hp_color(max(0, target["hp"]), target["max_hp"])
            lines.append(f"  {C.c(tname, C.BOLD)}:{profile_str} {C.c(actual_dmg, C.BYELLOW)} dmg → HP {old_hp}→{C.c(max(0, target['hp']), hp_c)}{save_result}{killed}")
        ...
        for grp_name in all_groups:
            ...
            for uid in uids:
                ...
                if u["alive"]:
                    ...
                    profile = self._describe_profile(u)
                    profile_tag = f" {C.c(profile, C.BCYAN)}" if profile else ""
                    lines.append(f"    {C.c(uid, C.BOLD, fc)}: HP {C.c(u['hp'], hp_c)}/{u['max_hp']} [{bar}]{profile_tag}{cover_mark}{rng_tag}")
                else:
    ...
    def _resolve_attack(
        ...
        if hit:
            ...
            damage, scaling = self._resolve_damage(
                raw_damage,
                attacker,
                defender,
            )
...
def main():
    ...
    def profile_arg(value: str) -> Dict:
        try:
        ...
        if not isinstance(profile, dict):
            raise argparse.ArgumentTypeError("--profile must be a JSON object")
        return profile
    ...
    p_add.add_argument(
        "--profile",
        type=profile_arg,
        help="Optional active-provider profile as a JSON object",
    )
