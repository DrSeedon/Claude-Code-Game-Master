...
sys.path.insert(0, str(PROJECT_ROOT / "modules" / "mass-combat" / "lib"))
...
class TestDamageProfiles:
    def test_vanilla_mass_combat_uses_raw_damage(self, engine):
        damage, details = engine._resolve_damage(
            12,
            {"combat_profile": {"offense": 1}},
            {"combat_profile": {"defense": 6}},
        )

        assert (damage, details) == (12, "")

    def test_active_provider_is_applied_without_module_coupling(self, engine):
        class DamageRuntime:
            @staticmethod
            def call_provider(capability, raw_damage, attacker, defender):
                assert capability == "combat.damage"
                return {"damage": raw_damage // 4, "details": "PROFILE[QUARTER]"}

        engine._module_runtime = DamageRuntime()
        ...
        assert (damage, details) == (3, "PROFILE[QUARTER]")
