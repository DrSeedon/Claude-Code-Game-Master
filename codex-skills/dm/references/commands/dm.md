...
If QUICK BUILD:
- Read `../specialists/create-character.md` and execute its quick-build path locally
- Standard array stats (15, 14, 13, 12, 10, 8)
- Basic equipment package
- Generic backstory ("wandering adventurer")

If CUSTOM BUILD:
- Read `../specialists/create-character.md` and execute its compact custom-build path locally
...
Always invoke `/dm-continue` for session startup — it handles module rules, narrator styles, campaign rules templates, and the full startup checklist.

---

### 🔒 MANDATORY STARTUP CHECKLIST (fallback if /dm-continue unavailable)
...
```
Then read `/tmp/dm-rules.md` completely. The adapter also prints campaign and session context that Claude previously injected with hooks.
...
Use the standard scene template from AGENTS.md.

---

## GAMEPLAY LOOP

Now you're playing. For every player action, follow the workflows in AGENTS.md:
